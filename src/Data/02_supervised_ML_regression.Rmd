...
bike_recipe <- recipe(price ~ frame_material + category_2 + model, data = bike_orderlines_tbl) %>%
...
workflow <- workflow(preprocessor = NULL, spec = NULL) %>% 
  workflows::add_recipe(bike_recipe) %>% 
  workflows::add_model(model_01_linear_lm_simple)

wf_recipe <- workflow(bike_recipe, model_01_linear_lm_simple)
fit(wf_recipe, train_transformed_tbl)

lm_fit <- fit(workflow, train_transformed_tbl)
